...
##    ]),
  ...
  (48,
   [
       ###(((prisoner_heroes_can_not_escape
       (assign, ":chance", 50),
       (try_begin),
        (eq, "$prisoner_heroes_can_not_escape", 1),
        (assign, ":chance", 0),
       (try_end),
       (call_script, "script_randomly_make_prisoner_heroes_escape_from_party", "p_main_party", ":chance"),
       ###)))
       # (call_script, "script_randomly_make_prisoner_heroes_escape_from_party", "p_main_party", 50), ###(((OLD)))
       (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
##         (party_slot_eq, ":center_no", slot_town_lord, "trp_player"),
         (assign, ":chance", 30),
         (try_begin),
   ...
   [           
    ...
    (try_begin),
        ...
        (try_begin),
            ...
            (party_remove_members, "p_npcs_days_on_mission_0", "$npc_to_rejoin_party", 1), ###(((npc_to_rejoin_party NEW)))
